...
class TwitchEventSubWS {
  ...
  async _reconnect() {
    if (this.reconnectUrl) {
      ...
      this.ws.on('close', (code, reason) => {
        console.log(`[EventSub-WS] Reconnected WebSocket closed: ${code}`);
        this._isConnected = false;
        this._clearKeepalive();
      });
      this.ws.on('error', (error) => {
   ...
   */
  onStreamOnline(broadcasterId, callback) {
   ...
   * @param {Function} callback
   */
  onStreamOffline(broadcasterId, callback) {
   ...
   * @param {Function} callback
   */
  onChannelFollow(broadcasterId, moderatorId, callback) {
   ...
   * @param {Function} callback
   */
  onChannelRaidTo(toBroadcasterId, callback) {
   ...
   * @param {Function} callback
   */
  onChannelRedemptionAdd(broadcasterId, callback) {
   ...
   * @param {Function} callback
   */
  onChannelSubscription(broadcasterId, callback) {
   ...
   * @param {Function} callback
   */
  onChannelSubscriptionMessage(broadcasterId, callback) {
   ...
   * @param {Function} callback
   */
  onChannelSubscriptionGift(broadcasterId, callback) {
   ...
   * @param {Function} callback
   */
  onChannelCheer(broadcasterId, callback) {
